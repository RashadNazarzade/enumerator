...
export const createObjectFeature = <const T extends Record<string, any>>(
  ...
  let cachedLoopItems: LoopItem<
    EnumerateValType,
    EnumerateValueMetadata | undefined
  >[];

  const getLoopItems = (): LoopItem<
    EnumerateValType,
    EnumerateValueMetadata | undefined
  >[] => {
  ...
  return {
    ...
    containsOneOf<Val extends EnumerateValue>(
      check: unknown,
      ...checkWithin: any[]
    ): check is Val {
    ...
    map<MappedValue>(
      callback: (
        item: LoopItem<EnumerateValType, EnumerateValueMetadata | undefined>,
        idx: number,
        items: LoopItem<EnumerateValType, EnumerateValueMetadata | undefined>[],
      ) => MappedValue,
    ): MappedValue[] {
      return getLoopItems().map(callback);
    },
