...
describe('Type Guards', () => {
  ...
  describe('isEnumerateValueWithMetadata', () => {
    test('should return true for valid metadata tuples', () => {
      ...
      expect(isEnumerateValueWithMetadata(['active', { description: 'test' }])).toBe(true);
      expect(isEnumerateValueWithMetadata([123, { category: 'numbers' }])).toBe(true);
    });
    ...
    test('should return false if second element is not metadata', () => {
      expect(isEnumerateValueWithMetadata(['active', 'not-object'])).toBe(false);
    ...
    test('should handle edge cases', () => {
      ...
      expect(isEnumerateValueWithMetadata(['active', { nested: { deep: true } }])).toBe(true);
    });
  });
