...
describe("Edge Cases & Bug Detection", () => {
  ...
  describe("Immutability Tests", () => {
    ...
    test("should not mutate original input", () => {
      ...
      const Status = enumerate(original);

      // Runtime: Original should remain unchanged
      expect(original.ACTIVE).toBe("active");
      expect(original.INACTIVE).toBe("inactive");

      // Runtime: Should be different objects
      expect(Status).not.toBe(original);

      // Type-level: Original type should be preserved
      expectTypeOf(original).toEqualTypeOf<{
        readonly ACTIVE: "active";
        readonly INACTIVE: "inactive";
      }>();

      // Type-level: Generated enum should have correct structure
      expectTypeOf(Status.ACTIVE).toHaveProperty("value");
      expectTypeOf(Status.ACTIVE).toHaveProperty("equals");
      expectTypeOf(Status.INACTIVE).toHaveProperty("value");
      expectTypeOf(Status.INACTIVE).toHaveProperty("equals");

      // Type-level: Values should preserve literal types
      expectTypeOf(Status.ACTIVE.value).toEqualTypeOf<"active">();
      expectTypeOf(Status.INACTIVE.value).toEqualTypeOf<"inactive">();

      // Type-level: Enum object should have object methods
      expectTypeOf(Status.contains).toBeFunction();
      expectTypeOf(Status.containsOneOf).toBeFunction();
      expectTypeOf(Status.forEach).toBeFunction();
      expectTypeOf(Status.map).toBeFunction();

      // Type-level: values array should have correct type
      expectTypeOf(Status.values).toEqualTypeOf<["active", "inactive"]>();

      // Type-level: Type guards should work correctly
      expectTypeOf(Status.contains).parameters.toEqualTypeOf<[unknown]>();
      expectTypeOf(Status.ACTIVE.equals).parameters.toEqualTypeOf<[unknown]>();
    });
  });
