...
test('EnumerateValMeta - should be a readonly tuple', () => {
  const valMeta: EnumerateValueWithMeta = ['active', { description: 'Active status' }] as const;
  
  expectTypeOf(valMeta).toEqualTypeOf<EnumerateValueWithMeta>();
  expectTypeOf(valMeta).toEqualTypeOf<readonly [EnumerateValType, EnumerateValueMetadata]>();
});
